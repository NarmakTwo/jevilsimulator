// FIGHT — obj_attackpress. Translated from the Black Knife sim (rory/sim/fightbar.js),
// which is itself a line-for-line read of the Chapter 1 dump.

import { choose } from './gml.js';

export const BOLT_SPEED = 8;
export const BOLT_START = 30;
export const ROW_PITCH = 38;
export const BAR_X = 2;
export const BAR_Y = 365;

function buildBolts(havechar) {
  const charbolt = havechar.map((h) => (h ? 1 : 0));
  const bolttotal = charbolt.reduce((a, b) => a + b, 0);
  if (bolttotal === 0) return [];
  const diff = 12; // one-button
  const boltuse = [0, 0, 0];
  const bolts = [];
  for (let i = 0; i < bolttotal; i += 1) {
    let c = choose(0, 1, 2);
    while (!havechar[c]) c = choose(0, 1, 2);
    while (boltuse[c] >= charbolt[c]) {
      c = choose(0, 1, 2);
      while (!havechar[c]) c = choose(0, 1, 2);
    }
    bolts.push({ char: c, frame: 0, alive: true, red: false });
    boltuse[c] += 1;
  }
  let boltxoff = 0;
  let lastbolt = -1;
  for (let i = 0; i < bolttotal; i += 1) {
    boltxoff += lastbolt;
    bolts[i].frame = BOLT_START + boltxoff;
    if (i < bolttotal - 1 && lastbolt !== 0 && bolts[i].char !== bolts[i + 1].char) {
      lastbolt = choose(0, diff, diff * 1.5);
      bolts[i].red = true;
    } else {
      lastbolt = choose(diff, diff * 1.5);
    }
  }
  return bolts;
}

/** `order` is party slots that chose FIGHT. Kris-only is `[0]`. */
export function createFightBar(order = [0]) {
  const havechar = [0, 1, 2].map((c) => (order.includes(c) ? 1 : 0));
  return {
    active: true,
    oneButton: true,
    boltx: 0,
    havechar,
    bolts: buildBolts(havechar),
    points: [0, 0, 0],
    pressbuffer: [0, 0, 0, 0],
    bursts: [],
    attacked: [false, false, false],
    // Ignore the Z that confirmed FIGHT until it is released.
    held: true,
    imagetimer: 0,
    afterimages: [],
    done: false,
    posttimer: 0,
    holdDone: false,
    fade: false,
    fadeamt: 0,
    applied: [false, false, false],
  };
}

function award(bar, bc, topclose) {
  const p = Math.abs(topclose);
  let gained;
  if (p === 0) gained = 150;
  else if (p === 1) gained = 120;
  else if (p === 2) gained = 110;
  else gained = 100 - p * 2;
  bar.points[bc] += gained;
  return { gained, critical: p === 0 };
}

function spawnBurst(bar, bolt, bc, critical) {
  bar.bursts.push({
    x: 80 + (bolt.frame - bar.boltx) * BOLT_SPEED,
    y: ROW_PITCH * bc,
    mag: critical ? 0.2 : 0.1,
    alpha: 1,
    xscale: 1,
    yscale: 1,
    critical,
    char: bc,
  });
}

function boltCheckOneButton(bar) {
  let qualify = -1;
  let dualId = -1;
  let topclose = 999;
  for (let i = 0; i < bar.bolts.length; i += 1) {
    const b = bar.bolts[i];
    if (!b.alive) continue;
    const close = b.frame - bar.boltx;
    if (close < 15 && close > -5) {
      if (close === topclose) dualId = i;
      if (close < topclose) {
        topclose = close;
        qualify = i;
      }
    }
  }
  for (let k = 0; k < 4; k += 1) bar.pressbuffer[k] = 5;
  if (qualify === -1) return 0;
  const hit = (idx) => {
    const b = bar.bolts[idx];
    const { critical } = award(bar, b.char, topclose);
    spawnBurst(bar, b, b.char, critical);
    b.alive = false;
  };
  hit(qualify);
  if (dualId !== -1) hit(dualId);
  return 1;
}

export function stepFightBar(bar, press = false) {
  if (!bar.active) return;

  for (const b of bar.bolts) {
    if (b.alive && b.frame - bar.boltx < -5) b.alive = false;
  }

  if (bar.imagetimer === 0) {
    for (const b of bar.bolts) {
      const close = b.frame - bar.boltx;
      if (b.alive && close >= 0) {
        bar.afterimages.push({
          x: 80 + close * BOLT_SPEED, y: ROW_PITCH * b.char, alpha: 0.4,
        });
      }
    }
  }

  for (let i = 0; i < 3; i += 1) {
    if (!bar.havechar[i] || bar.attacked[i]) continue;
    if (!bar.bolts.some((b) => b.alive && b.char === i)) {
      bar.attacked[i] = true;
      break;
    }
  }

  if (press && !bar.held) boltCheckOneButton(bar);
  bar.held = press;

  bar.imagetimer = bar.imagetimer > 0 ? 0 : bar.imagetimer + 1;
  bar.boltx += 1;
  for (let k = 0; k < 4; k += 1) bar.pressbuffer[k] -= 1;

  for (const s of bar.bursts) {
    s.alpha -= 0.1;
    s.xscale += s.mag;
    s.yscale += s.mag;
    s.x += ((1 - 10 * s.xscale) * s.mag) / 2.7;
    s.y += ((1 - 38 * s.yscale) * s.mag) / 2.5;
  }
  bar.bursts = bar.bursts.filter((s) => s.alpha >= 0);
  for (const a of bar.afterimages) a.alpha -= 0.05;
  bar.afterimages = bar.afterimages.filter((a) => a.alpha > 0);

  if (bar.attacked.every((a, i) => a || !bar.havechar[i])) bar.done = true;
  if (bar.done) bar.posttimer += 1;
  bar.holdDone = bar.posttimer > 50;
  if (bar.holdDone) {
    bar.fade = true;
    bar.fadeamt += 0.08;
  }
}

export function fightTp(accuracy) {
  return Math.round(accuracy / 10);
}
